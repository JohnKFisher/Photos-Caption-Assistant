...
private enum CaptionWorkflowRunError: LocalizedError {
    ...
    var errorDescription: String? {
        switch self {
        case .albumListingUnavailable:
            return "\(AppPresentation.queuedAlbumsTitle) requires a Photos client that can list user albums."
        case let .incompleteConfiguration(queueItems):
            return "\(AppPresentation.queuedAlbumsTitle) is missing configured albums for: \(queueItems.joined(separator: ", ")). Repair the queue before starting the run."
        case let .duplicateConfiguredAlbums(queueItems):
            return "\(AppPresentation.queuedAlbumsTitle) queue items must use different albums. Duplicate selections were found for: \(queueItems.joined(separator: ", "))."
        case let .missingConfiguredAlbum(queueItem, savedAlbumName):
            return "\(AppPresentation.queuedAlbumsTitle) could not find the configured album for queue item \(queueItem). Saved selection: \"\(savedAlbumName)\". Repair the queue before starting the run."
        case let .noEligibleItems(filtered, albumNames):
            ...
            return "\(AppPresentation.queuedAlbumsTitle) found no eligible items\(scopeText) in \"\(albumList)\"."
        }
    }
}
...
actor DefaultPreviewRenderer: PreviewRendering {
    ...
    func persistPreviewFile(
        ...
        let previewRoot = AppStoragePaths.make(fileManager: fileManager).previewTempRoot
...
public final class RunCoordinator {
    ...
    public func run(
        ...
        guard capabilities.qwenModelAvailable else {
            ...
            case .notInstalled:
                ollamaError = "Ollama is not installed yet. Open the official download page from the setup card, install Ollama, then click Re-check Setup."
            case .installedNotRunning:
                ollamaError = "Ollama is installed but not running yet."
        ...
        func captionWorkflowChunkCollectionStatus(
            ...
            "\(AppPresentation.queuedAlbumsTitle): collecting next chunk for \(queueItem.displayName) (\(min(discovered, target))/\(target) discovered)."
        }
        ...
        func loadCaptionWorkflowQueueSnapshot(
            ...
            for attempt in 1...Self.captionWorkflowStageLoadMaxAttempts {
                if attempt > 1 {
                    callbacks.onStatusChanged(
                        "\(AppPresentation.queuedAlbumsTitle): retrying \(queueItemPlan.displayName) (attempt \(attempt) of \(Self.captionWorkflowStageLoadMaxAttempts))."
                    )
                }
                ...
                do {
                    ...
                    if snapshot.isEmpty, attempt < emptyConfirmationLimit {
                        callbacks.onStatusChanged(
                            "\(AppPresentation.queuedAlbumsTitle): waiting for Photos to refresh \(refreshedQueueItem.displayName) before deciding it is empty."
                        )
                    ...
                    callbacks.onStatusChanged(
                        "\(AppPresentation.queuedAlbumsTitle): stage handoff for \(queueItemPlan.displayName) failed (\(error.localizedDescription)). Waiting briefly and retrying."
                    )
                    guard await waitForCaptionWorkflowRetryDelay() else {
                        break
                    }
            ...
            throw lastError ?? PhotosAppleScriptError.scriptFailed(
                message: "\(AppPresentation.queuedAlbumsTitle) failed while loading \(queueItemPlan.displayName)."
            )
        }
        ...
        func captionWorkflowSkipStatus(
            ...
            return "\(AppPresentation.queuedAlbumsTitle): \(queueItem.displayName) has no eligible items\(suffix)."
        }
        ...
        func captionWorkflowProcessingStatus(
            ...
            if skippedQueueItems.isEmpty {
                prefix = "\(AppPresentation.queuedAlbumsTitle): processing"
            } else {
                let skippedNames = skippedQueueItems.joined(separator: ", ")
                prefix = "\(AppPresentation.queuedAlbumsTitle): \(skippedNames) had no eligible items, now processing"
            }
        ...
        func runCaptionWorkflow() async -> RunSummary {
            ...
            do {
                ...
                errors.append("\(AppPresentation.queuedAlbumsTitle) failed while refreshing albums: \(error.localizedDescription)")
                return await completeRun()
            }
            ...
            for queueItem in preflightQueue {
                ...
                callbacks.onStatusChanged("\(AppPresentation.queuedAlbumsTitle): refreshing \(queueItem.displayName).")
                ...
                if let chunkedFastTraversalWriter {
                    ...
                    while !isCancelled {
                        ...
                        do {
                            ...
                            errors.append("\(AppPresentation.queuedAlbumsTitle) failed while loading \(queueItem.displayName): \(error.localizedDescription)")
                            return await completeRun()
                        }

...
public actor PhotosAppleScriptClient: PhotosWriter, PhotosProcessMonitoring, PhotosLifecycleControlling, PhotoPreviewSource, PhotoPreviewDataSource, IncrementalPhotosWriter, BatchMetadataPhotosWriter, BatchWritePhotosWriter, AlbumListingPhotosSource {
    ...
    public func enumerate(scope: ScopeSource, dateRange: CaptureDateRange?) async throws -> [MediaAsset] {
        if case .captionWorkflow = scope {
            throw PhotosAppleScriptError.scriptFailed(
                message: "\(AppPresentation.queuedAlbumsTitle) must be resolved stage-by-stage by RunCoordinator before direct enumeration."
            )
        }
    ...
    public func count(scope: ScopeSource) async throws -> Int {
        if case .captionWorkflow = scope {
            throw PhotosAppleScriptError.scriptFailed(
                message: "\(AppPresentation.queuedAlbumsTitle) must be resolved stage-by-stage by RunCoordinator before direct counting."
            )
        }
    ...
    public func enumerate(scope: ScopeSource, offset: Int, limit: Int) async throws -> [MediaAsset] {
        if case .captionWorkflow = scope {
            throw PhotosAppleScriptError.scriptFailed(
                message: "\(AppPresentation.queuedAlbumsTitle) must be resolved stage-by-stage by RunCoordinator before paged enumeration."
            )
        }
    ...
    private func exportAssetToTemporaryURLViaAppleScript(id: String, kind: MediaKind) throws -> URL {
        let exportRoot = AppStoragePaths.make(fileManager: fileManager).photoExportTempRoot
    ...
    private func makeTemporaryVideoOutputURL(assetID: String, fileExtension: String) throws -> URL {
        let exportRoot = AppStoragePaths.make(fileManager: fileManager).videoExportTempRoot
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
        try fileManager.createDirectory(at: exportRoot, withIntermediateDirectories: true)
    ...
    private func writePreviewImageDataToTemporaryURL(_ jpegData: Data, assetID: String) -> URL? {
        ...
        let previewRoot = AppStoragePaths.make(fileManager: fileManager).photoPreviewTempRoot
            .appendingPathComponent(UUID().uuidString, isDirectory: true)
